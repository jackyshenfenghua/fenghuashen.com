...
const heroProof = html.match(/<aside class="hero-proof"[\s\S]*?<\/aside>/)?.[0] || '';
assert.match(heroProof, /心理咨询师 · 高级工程师 · 高级疗愈师/, 'hero proof must show the requested professional identity labels');
assert.doesNotMatch(heroProof, /Power Platform|Power BI/, 'hero proof must not show the old technology labels');
